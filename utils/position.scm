...
(define (calc-new-position position direction)
  (cond ((= direction 'up) (make-position (get-x-position position)
                                          (- (get-y-position position)
                                             1)))
        ((= direction 'down) (make-position (get-x-position position)
                                            (+ (get-y-position position)
                                               1)))
        ((= direction 'left) (make-position (- (get-x-position position) 
                                               1)
                                            (get-y-position position)))
        ((= direction 'right) (make-position (+ (get-x-position position) 
                                                1)
                                             (get-y-position position)))
